<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-teletalk-red leading-tight">Super Admin Control Panel</h2>
    </x-slot>
    ...
    <div class="py-6 sm:py-12 max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            {{-- Active Files Count --}}
            <div class="bg-white shadow rounded-lg p-5 sm:p-6 text-center border-t-4 border-teletalk-red">
                <h3 class="text-gray-500 font-bold uppercase text-sm tracking-wide">Total Active Files</h3>
                <p class="text-4xl font-bold text-gray-900 mt-2">{{ $allActiveFiles }}</p>
            </div>

            {{-- System Management Card --}}
            <div
                class="sm:col-span-1 md:col-span-2 bg-white shadow rounded-lg p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-t-4 border-gray-800">
                <div>
                    <h3 class="font-bold text-xl text-gray-800">System Management</h3>
                    <p class="text-sm text-gray-500 mt-1">Manage users, approvals, and system reports.</p>
                </div>
                <div class="flex flex-wrap gap-3 w-full sm:w-auto">
                    <a href="{{ route('admin.users.index') }}"
                        class="relative flex-1 sm:flex-none text-center bg-gray-800 text-white px-5 py-3 rounded-lg hover:bg-black transition-all transform hover:-translate-y-0.5 active:scale-95 font-bold shadow-lg select-none text-sm">
                        Manage Users
                    ...
                    <a href="{{ route('admin.reports.index') }}"
                        class="flex-1 sm:flex-none text-center bg-teletalk-green text-white px-5 py-3 rounded-lg hover:bg-green-800 transition-all transform hover:-translate-y-0.5 active:scale-95 font-bold shadow-lg whitespace-nowrap select-none text-sm">
                        View &amp; Export Reports
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                @foreach($departments as $dept)
                    @php
                     ...
                     @endphp
                    <div class="border rounded-lg p-3 sm:p-4 shadow-sm {{ $bgClass }} transition hover:shadow-md">
                        <h4 class="text-xs sm:text-sm font-medium text-gray-600 truncate">{{ $dept->name }}</h4>
                        <div class="mt-2 flex items-baseline gap-1">
                            <span class="text-2xl sm:text-3xl font-bold {{ $textClass }}">{{ $count }}</span>
                            <span class="text-xs sm:text-sm text-gray-500">active files</span>
                        </div>
                    </div>
                @endforeach
            </div>
                        ...
                        if (data.success) {
                            ...
                            let html = `
                                <div class="bg-gray-50 p-4 sm:p-6 rounded-lg border border-gray-200">
                                    ...
                                    <h5 class="font-bold text-gray-700 mb-4 uppercase text-sm tracking-widest">Global Audit Trail</h5>
                            ...
                            resultsContainer.innerHTML = `
                                <div class="p-6 sm:p-8 bg-red-50 border border-red-200 rounded-xl text-red-700 text-center shadow-inner">
                                    ...
                                    <p class="font-bold text-xl sm:text-2xl mb-2">Audit Entry Not Found</p>
                                    <p class="text-gray-600 text-base">Tracking ID: <span class="text-red-600 font-mono font-bold px-2 py-1 bg-red-100 rounded">'${query}'</span> does not exist in the organizational Dak database.</p>
                                </div>
                            `;
                        }
